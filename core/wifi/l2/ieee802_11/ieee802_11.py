...
class IEEE802_11:
    ...
    class Parsers:
        ...
        def mac_header(frame, offset):
            try:
                ...
                frame_type_name, frame_subtype_name = L2.IEEE802_11.Parsers._get_frame_type_subtype_name(frame_type, frame_subtype)
        ...
        def eapol(frame, offset):
            ...
            return {
                "AuthenticationVersion": auth_ver, "EAPOLType": eapol_type, "HeaderLength": length,
    ...
    class Management:
        class build:
            ...
            def deauthentication(dst_mac, src_mac, bssid=None, reason_code=0x0007, **kwargs):
                radiotap = L2.Radiotap.header(**kwargs)
                frame_control = struct.pack("<H", 0x00C0)  # Management + Deauthentication
                mac_header = L2.IEEE802_11.Management.build.mac_header(
            ...
            def probe_request(src_mac, dst_mac="ff:ff:ff:ff:ff:ff", ssid="", rates=None, **kwargs):
                ...
                tagged_params = L2.IEEE802_11.Management.build.tagged_parameters(
            ...
            def association_request(dst_mac, src_mac, ssid, rates=None, capabilities=0x0431, **kwargs):
                ...
                def parser(frame, offset):
                    tagged_parameters, tagged_parameters_offset = L2.IEEE802_11.Parsers.tagged_parameters(frame, offset)
                    return tagged_parameters
    ...
    class Data:
        ...
        def parser(frame, offset):
            body = {}
            llc, llc_offset = L2.IEEE802_11.Parsers.llc(frame, offset)
            body["LLC"] = llc
            if llc.get("Type", "") == "0x888e":
                eapol, eapol_offset = L2.IEEE802_11.Parsers.eapol(frame, llc_offset)
                body["EAPOL"] = eapol
            return body
    ...
    def frames_parser(raw_frame: bytes) -> dict:
        ...
        rth, rth_len = L2.Radiotap.parser(raw_frame)
        mac_hdr, mac_hdr_offset = L2.IEEE802_11.Parsers.mac_header(raw_frame, rth_len)
        ...
        try:
            ...
            if frame_type == 0:
                 body = L2.IEEE802_11.Management.parser(raw_frame, mac_hdr_offset, parsed_frame)
                 parsed_frame["Body"] = body
            elif frame_type == 1:
                parsed_frame["Body"] = {"Unsupported parser!"}
            elif frame_type == 2:
                body = L2.IEEE802_11.Data.parser(raw_frame, mac_hdr_offset)
                parsed_frame["Body"] = body
            else:
                parsed_frame["Body"] = {"Unsupported parser!"}
        except Exception as error:
...
def sniff(ifname: str = None, store_filter: str = "", display_filter: str = "", 
          output_file: str = None, count: int = 0, timeout: int = None):
    if not ifname:
        raise ValueError("Interface name is required")
        
    sock = L2.create_socket(ifname)
    ...
    try:
        ...
        while True:
            try:
                ...
                parsed_frame = L2.IEEE802_11.frames_parser(frame)
                store_result, display_result = L2.IEEE802_11.frames_filter(store_filter, display_filter, parsed_frame)
    ...
    class WPA2Personal:
        @staticmethod
        def eapol_capture(ifname: str = None, bssid: str = None, mac: str = None, output_file: str = None):
            pass
        ...
        def generate_22000(eapol_msg1_hex, eapol_msg2_hex, output_file="hashcat.22000"):
            ...
            rth1, rth_len1 = L2.Radiotap.parser(msg1)
            mac_hdr1, mac_hdr_offset1 = L2.IEEE802_11.Parsers.mac_header(msg1, rth_len1)
            body1 = L2.IEEE802_11.Data.parser(msg1, mac_hdr_offset1)

            rth2, rth_len2 = L2.Radiotap.parser(msg2)
            mac_hdr2, mac_hdr_offset2 = L2.IEEE802_11.Parsers.mac_header(msg2, rth_len2)
            body2 = L2.IEEE802_11.Data.parser(msg2, mac_hdr_offset2)

            ap_mac = mac_hdr1.get("BSSID", "").replace(":", "")
            sta_mac = mac_hdr1.get("MACSource", "").replace(":", "")

            nonce = body2["EAPOL"].get("KeyNonce", "")
            mic = body2.get("KeyMIC", "")
            eapol2_len = len(msg2[mac_hdr_offset2:])

            line = f"$WPAPSK${ap_mac}${sta_mac}${nonce}${mic}${eapol2_len}:{msg2[mac_hdr_offset2:].hex()}\n"
