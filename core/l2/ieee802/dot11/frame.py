...
def parse(frame: bytes, offset: int = 0) -> dict:
    ...
    try:
        with ParseContext(frame, offset) as ctx:
            ...
            fcs = unpack(parser=detect_fcs, metadata=False)
            ctx.set("fcs", fcs)

            mac_hdr = unpack(parser=dot11_parsers.mac_header)
            ...
            ctx.set(
                "body",
                 unpack(parser=dot11_parsers.body_dispatch)
            )
