...
def main():
    ...
    set_frequency_parser.add_argument("channel_width", type=int, default=20, help="channel width")
    ...
    scan_monitor_parser.add_argument("--channel-hopping", default=True, help="Channel hopping, default True.")
    scan_monitor_parser.add_argument("--timeout", type=float, default=None, help="Scan timeout, default None.")
    ...
    generate_channel_hopping_config.add_argument("bands", nargs="+", type=float, default=[2.4, 5], help="bands to tour, e.g: 2.4 5 6")
    generate_channel_hopping_config.add_argument("--output", "-o", type=str, required=True, help="Output filename for channel hopping config.")
    generate_channel_hopping_config.add_argument("--channel-width", type=int, default=20, help="Channel width.")
    generate_channel_hopping_config.add_argument("--dwell", type=float, default=4000, help="Dwell time (milliseconds) float.")
    ...
    if args.command == "list-interfaces":
        ...
        operations.set_frequency(args.ifname, args.frequency_mhz)
    elif args.command == "generate-channel-hopping-config":
         operations.generate_channel_hopping_config(bands=args.bands, channel_width=args.channel_width, dwell=args.dwell, output_filename=args.output)
    elif args.command == "channel-hopping":
        operations.channel_hopper(args.ifname, args.channel_hopping_config_filename, args.allowed, args.disallowed)
        ...
        operations.generate_22000(
         ...
         operations.scan_monitor(ifname=args.ifname, dlt=args.dlt, channel_hopping=args.channel_hopping, timeout=args.timeout)
